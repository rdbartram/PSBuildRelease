...
task CompileClasses @{
    ...
    Jobs    = {
        ...
        $files = @(
            ...
            @{file = "$ProjectName-Classes.psm1"; filter = "*.psm1"; exclude = "*-attribute.psm1"; DependentModules = @("$BuildOutput\attributes.psm1") }
        )
        ...
        try {
            foreach ($file in $files) {
                ...
                foreach ($dependentModule in $dependentModules) {
                    $classScript += "using module $($dependentModule.replace($BuildOutput,"."))"
                }
