task GitVersion -before CreateModuleManifest, IncrementScriptVersion -If ((get-command gitversion -ErrorAction SilentlyContinue) -and ($null -eq $env:GITVERSION_SEMVER)) {
...
task FakeGitVersion -before CreateModuleManifest, IncrementScriptVersion -If (-not (get-command gitversion -ErrorAction SilentlyContinue) -and ($null -eq $env:GITVERSION_SEMVER)) {
